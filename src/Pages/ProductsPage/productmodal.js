...
const ProductModal = ({
 ...
 const validateRequired = (field, value) => {
  ...
  if (showSuccess) {
    return (
      <Modal open={open} onClose={handleCloseSuccess} aria-labelledby="success-message-modal" sx={{ display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        <Box
  ...
  // Main Form Modal
  return (
    <Modal open={open} onClose={handleClose} aria-labelledby="product-form-modal" sx={{ display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
      <Box
        onClick={(e) => e.stopPropagation()}
        sx={{
          width: "800px",
      ...
      >
        ...
        <Box sx={{ px: 5, pb: 5 }} component="form" onSubmit={handleSubmit}>
          ...
          >
            {isSubmitting ? (
              <Button disabled variant="contained" sx={{ color:'#fcfcfc',borderRadius: "50px", px: 5, py: 1.5,
              '&.Mui-disabled': {
              color: '#fcfcfc',
